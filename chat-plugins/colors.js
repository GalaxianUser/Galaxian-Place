...
function updateColor() {
	...
	request('http://play.pokemonshowdown.com/customcss.php?server=galaxy&invalidate', function callback(error, res, body) {
		if (error) return console.log('updateColor error: ' + error);
	});
}
Galaxy.updateColor = updateColor;
...
exports.commands = {
	...
	hex: function(target, room, user) {
		...
		return this.sendReplyBox('The hexcode for ' + '<b><font color="' + Galaxy.hashColor(target) + '">' + target + '</font></b> is: ' +  '<font color="' + Galaxy.hashColor(target) + '">' + Galaxy.hashColor(target) + '</font>');
	},
	...
	chatcolor: function (target, room, user) {
		...
		if (!this.can('vip') || !this.runBroadcast()) return;
		...
		this.add('|raw|' + "<div class='chat'>" + "<small>" + group + "</small>" + "<button name='parseCommand' value='/user " + user.name + "' style='" + style + "'>" + "<b><font color='" + Galaxy.hashColor(user.userid) + "'>" + user.name + ":</font></b>" + '<font color="' + targets[0].toLowerCase().replace(/[^#a-z0-9]+/g, '') + '">' + Tools.escapeHTML(targets.slice(1).join(",")) + '</font></div>');
	},
