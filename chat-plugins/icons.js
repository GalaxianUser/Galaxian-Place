...
Galaxy.updateIcons = updateIcons;
...
exports.commands = {
	...
	icon: function (target, room, user) {
		...
		this.sendReply("|raw|You have given <b><font color=" + Galaxy.hashColor(Tools.escapeHTML(target[0])) + ">" + Tools.escapeHTML(target[0]) + "</font></b> an icon.");
		Rooms('staff').add('|raw|<b><font color="' + Galaxy.hashColor(Tools.escapeHTML(target[0])) + '">' + Tools.escapeHTML(target[0]) + '</font> has received an icon from ' + Tools.escapeHTML(user.name) + '.</b>').update();
